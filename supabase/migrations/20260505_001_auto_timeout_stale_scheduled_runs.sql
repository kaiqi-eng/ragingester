-- Auto-timeout stale scheduled runs so one zombie "running" row does not block the queue forever.
-- A scheduled run is considered stale if it has been in running status for more than 30 minutes.
...
begin
  ...
  -- Auto-fail stale running scheduled runs so they don't block new claims forever.
  update public.collection_runs
  set
    status = 'failed',
    ended_at = now(),
    error = 'scheduled run timed out in queue claim',
    error_payload = jsonb_build_object(
      'name', 'RunTimeoutError',
      'message', 'scheduled run exceeded 30 minute running timeout'
    )
